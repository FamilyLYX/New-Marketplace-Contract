    ...
    Withdrawable
{
    ...
    event ParticipantChanged(
        address indexed oldParticipant,
    ...
    function setRoyaltiesThresholdPoints(
        uint32 newRoyaltiesThresholdPoints
    ) external onlyOwner {
        ...
        if (royaltiesThresholdPoints != newRoyaltiesThresholdPoints) {
            ...
            emit RoyaltiesThresholdPointsChanged(
                old,
                newRoyaltiesThresholdPoints
            );
        }
    }

    // function _calculateFeeWithDiscount(
    //     address seller,
    //     uint256 totalPrice
    // ) internal view returns (uint256) {
    //     uint32 discountPoints = Points.multiply(
    //         feePoints,
    //         participant.feeDiscountFor(seller)
    //     );
    //     assert(discountPoints <= feePoints);
    //     return Points.realize(totalPrice, feePoints - discountPoints);
    // }

    function _calculateFee(uint256 totalPrice) internal view returns (uint256) {
        return Points.realize(totalPrice, feePoints);
    }
