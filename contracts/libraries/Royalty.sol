...
library Royalties {
    function setRoyalties(address asset, RoyaltiesInfo memory info) internal {
        ...
        bytes memory entriesData = IERC725Y(asset).getData(
        ...
        if (entryIndex < 0) {
            // adding new entry
            entriesData = bytes.concat(
                entriesData,
                _encodeRoyaltiesEntry(info)
            );
        } else {
    ...
    function royalties(
        address asset
    ) internal view returns (RoyaltiesInfo[] memory) {
        bytes memory value = IERC725Y(asset).getData(
            _LSP18_ROYALTIES_RECIPIENTS_KEY
        );
        return _decodeRoyalties(value);
    }

    function royaltiesPaymentEnforced(
        address asset
    ) internal view returns (bool) {
        bytes memory value = IERC725Y(asset).getData(
            _LSP18_ROYALTIES_ENFORCE_PAYMENT
        );
        return value.length > 0 && value[0] != 0;
    }

    function _indexOfRoyaltiesEntry(
        RoyaltiesInfo[] memory entries,
        address recipient
    ...
    function _decodeRoyalties(
        bytes memory data
    ) private pure returns (RoyaltiesInfo[] memory) {
        ...
        while (k < dataLength) {
            ...
            assembly {
                ...
                recipient := div(
                    mload(add(add(data, 0x20), add(k, 6))),
            ...
            if (
                length >=
                4 /* interfaceId */ + 20 /* recipient */ + 4 /* points */
            ) {
                assembly {
    ...
    function _encodeRoyaltiesEntry(
        RoyaltiesInfo memory entry
